...
2. Request local tools when you need more context.
3. If the implementation is clear, propose code changes as an apply_patch patch.
4. Read the tool observation. SpecForge applies accepted patches automatically
   and runs project checks automatically after each applied patch.
5. If checks fail, inspect the failure and propose another patch.
6. Stop only when the implementation is complete or you cannot make progress.
